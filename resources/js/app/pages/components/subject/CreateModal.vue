<template>
	<Dialog v-model:visible="subjectDialog" :style="{width: '450px'}" header="Subject details" :modal="true" class="p-fluid">
		<div class="field">
			...
			<InputText id="name" v-model="subject.name" autofocus :class="{'p-invalid': submitted && !subject.name}" />
			<small class="p-error" v-if="submitted && !subject.name">Please enter the subject Name.</small>
		</div>
		<div class="field">
...
export default {
    ...
    mounted() 
    { 
        window.mitt.on("subjectDialog", data => this.subjectDialog = data)
    ...
    methods:{
		saveSubject() 
		{
            ...
            axios.post('/api/subjects', this.subject)
				.then(res => {
					console.log('createdSubject', res.data.subject)
					window.mitt.emit("createdSubject", res.data.subject)
			...
			this.subject = { name: '', masters: [], diplomas: [], specialities: [] }
		},
        searchMaster(event) {
            setTimeout(() => {
